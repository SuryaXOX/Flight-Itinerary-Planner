...
import java.util.NoSuchElementException~WE
...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    PriorityQueue<Path> P_Queue = new PriorityQueue<Path>();

    LinkedList<Vertex> L_List = new LinkedList<Vertex>();

    L_List.add(vertices.get(start));

    P_Queue.add(new Path(vertices.get(start)));

    while (P_Queue.peek() != null) {
      if (P_Queue.peek().end.data.equals(end))
        return P_Queue.poll();

      Path Queue = new Path(P_Queue.poll());
      LinkedList<Edge> Edges = new LinkedList<Edge>();
      for (int i = 0; i < Queue.end.edgesLeaving.size(); i++) {
        Edges.add(i, Queue.end.edgesLeaving.get(i));
      }
      for (int j = 0; j < Queue.end.edgesLeaving.size(); j++) {

        Path Path = new Path(Queue);
        if (!L_List.contains(Edges.peekFirst().target)) {

          L_List.add(Edges.peekFirst().target);
          Path.extend(Edges.pop());
          P_Queue.add(Path);
